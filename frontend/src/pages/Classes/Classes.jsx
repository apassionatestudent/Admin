...
//    min attribute - "can't setup today and yesterday" means the earliest
//    allowed start_date is tomorrow, matching the backend's validateBatchDates
...
// => Client-side mirror of the backend's validateBatchDates - the min
//    attribute on the date inputs guides the native picker, but this form
//    isn't a native <form> submission, so the browser's own min/max
//    enforcement never fires on submit. This catches a typed-in invalid
//    date before the request even goes out, matching the backend's rules.
...
// => Initial state for the Add SHS Batch form - separate from EMPTY_CLASS_FORM
...
export default function Classes() {
  ...
  // => Add Class modal state - now handles both TESDA and SHS, picked via
  //    createType. formError/formSaving are shared since only one type's
  //    form is ever showing/submitting at a time.
  ...
  // => Fetch form options (courses, trainers, clusters) when modal opens
  ...
  const handleCreateClass = async () => {
    ...
    try {
      ...
      //    earlier in handleRestoreFacility/handleRestoreTrainer
        ...
        <div className="adm-modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) handleCloseModal(); }}>
          <div className="adm-modal-box adm-modal-box--form">
            ...
            <div className="adm-modal-body">
              ...
              {/* => Program Type toggle - picks which form below is shown
                     and which endpoint Create Batch posts to. Reuses the
                     same pill styling as the Type filter row for visual
                     consistency, no new CSS needed. */}
                ...
                <div className="adm-modal-type-toggle">
                  {['TESDA', 'SHS'].map(t => (
                    <button
                      ...
                      onClick={() => { setCreateType(t); setFormError(null); }}
                      disabled={formSaving}
                    >
                      {t}
                    </button>
              ...
              {createType === 'TESDA' ? (
                <>
                  ...
                  {/* => Trainer (optional - nullable per schema) - assigning
                         an unqualified trainer here is a hard error, not a
                         confirm prompt, since TESDA accreditation has no
                         substitute path */}
                  <div className="adm-form-group">
                  ...
                  {/* => Start Date + End Date side by side - both optional
                         now, matching SHS: an admin may be pooling students
                         for enrollment before a firm schedule exists */}
                ...
                <>
                  ...
                  {/* => Start Date + End Date side by side - both optional,
                         ...
                         unknown while Pending, end_date stays open while Ongoing */}
                  <div className="adm-form-row">
                    <div className="adm-form-group">
                      <label className="adm-form-label">Start Date <span className="adm-form-optional">(optional)</span></label>
                      <input
                  ...
                  {/* => Required Students + Max Students side by side -
                         matches TESDA's layout, was missing entirely before,
                         which meant every SHS batch creation failed since
                ...
                <tbody>
                  {applyFacilityFilters(facilities).map((f, idx) => (
                    <tr
                      ...
                      onClick={facilityViewMode === 'deleted' ? undefined : () => navigate(`/dashboard/facilities/${f.public_id}`)}
                ...
                <tbody>
                  {applyTrainerFilters(trainers).map((i, idx) => (
                    <tr
                      ...
                      onClick={trainerViewMode === 'deleted' ? undefined : () => navigate(`/dashboard/trainers/${i.public_id}`)}
