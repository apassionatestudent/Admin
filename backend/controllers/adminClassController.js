...
export const searchClassesController = async (req, res) => {
  const { course_name, instructor_name, status, sector, program_type, cluster, start_date_from, start_date_to } = req.query;
  try {
    const results = await searchClassesService({
      course_name, instructor_name, status, sector, program_type, cluster, start_date_from, start_date_to,
    });
