  ...
  // => Resolves a trainer_id to its display name - used when building the
  //    Batch Information edit diff, since the edit payload only carries the
  //    new trainer_id (a number), not a name to compare against the old one.
  export const getTrainerNameById = async (pool, trainerId) => {
    if (!trainerId) return null;
    const result = await pool.query(
      `SELECT trainer_full_name FROM trainers WHERE trainer_id = $1`,
      [trainerId]
    );
    return result.rows[0]?.trainer_full_name ?? null;
  };
    ...
    try {
      ...
      const updateResult = await client.query(
        `UPDATE tesda_enrollments
          ...
          WHERE public_id = $2
          RETURNING public_id, enrollment_id, batch_id, status`,
