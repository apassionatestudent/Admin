// utils/sendStudentPasswordResetEmail.js

import { resend } from '../config/email.js';

// => Points at the public/student frontend, same env var
//    sendEnrollmentStatusEmail.js already uses for student-facing links
const STUDENT_APP_URL = process.env.STUDENT_APP_URL || 'http://localhost:5173/';

// => Sent when an admin clicks "Reset Password" on a student's detail page.
// => Uses a query string token, not a path param, since Site's SetPassword
//    page reads it via useSearchParams().get('token')
export async function sendStudentPasswordResetEmail({ toEmail, studentName, rawToken }) {
    const resetLink = `${STUDENT_APP_URL}set-password?token=${rawToken}`;

    const { error } = await resend.emails.send({
        from: process.env.RESEND_FROM_EMAIL,
        to: toEmail,
        subject: 'PrimeEnroll Digital - Reset Your Password',
        html: `
            <!DOCTYPE html>
            <html>
                                ...
                                <tr>
                                    <td style="padding:36px;">
                                        ...
                                        <p style="margin:0 0 20px; font-size:15px; line-height:1.6; color:#3a3540;">
                                            Our staff has requested a password reset for your PrimeEnroll student account.
                                        ...
                                        <p style="margin:0; font-size:13px; color:#6b6375; border-top:1px solid #e5e4e7; padding-top:16px;">
                                            This link expires in 10 minutes. If you weren't expecting this, contact
                                            our office, your current password remains unchanged until you complete
                                            this reset.
                                        </p>
                                    </td>
                                </tr>
    ...
    // => Resend doesn't throw on failure, same check as every other
    //    send*Email.js file in this project
    if (error) {
        console.error('Resend student password reset email failed:', error);
        throw new Error('Failed to send student password reset email');
    }
}